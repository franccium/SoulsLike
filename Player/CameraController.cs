...
public partial class CameraController : Node
{
    ...
    public override void _Ready()
    {
        ...
        CameraContainerNode = GetNode<Node3D>("CameraContainer");
        CameraPivotNode = GetNode<Node3D>("CameraContainer/CameraPivot");
        BodyNode = GetParent<Node3D>();

        CameraPanDesiredVerticalSpeed = -3f;
        CameraPanDesiredHorizontalSpeed = 0.1f;
        CameraPanBlendSpeed = 10f;

        CameraHorizontalMoveSpeed = 15f;
        CameraVerticalMoveSpeed = 4f;
        CameraMovementDampening = 0.6f;
        CameraChangeSpeed = 0.3f;
        MaxAllowedDistance = 2f;

        CameraNode = CameraPivotNode.GetNode<Camera3D>("Camera3D");
        CameraCollisionRay = CameraPivotNode.GetNode<RayCast3D>("CameraCollisionRayCast");
        CameraZoomDesiredSpeed = 1f;
        CameraZoomBlendSpeed = 5f;
        CameraFastZoomBlendSpeed = 20f;

        MinimumZoomDistance = 2f;
        MaximumZoomDistance = 15f;
        RaycastImpactOffset = 0.5f;
    }
